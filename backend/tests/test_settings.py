...
def test_an_empty_database_falls_back_to_declared_defaults(db):
    assert settings_store.resolve(db, "monitor.decision_minutes_before_close") == 15
    assert settings_store.resolve(db, "monitor.enabled") is True
    assert _field(settings_store.public_view(db), "monitor.decision_minutes_before_close")["source"] == "environment"
...
def test_an_override_wins_and_is_reported_as_such(db):
    ...
    assert settings_store.resolve(db, "execution.max_total_allocation") == 50_000
    field = _field(settings_store.public_view(db), "execution.max_total_allocation")
    assert field["value"] == 50_000
...
def test_values_are_coerced_and_bounded(db):
    ...
    with pytest.raises(ValueError, match="expected a number"):
        settings_store.apply_updates(db, {"execution.max_order_notional": "soon"})
    with pytest.raises(ValueError, match="must be one of"):
        settings_store.apply_updates(db, {"data.market_data_provider": "yfinance"})


def test_a_secret_never_comes_back_out(db):
    settings_store.apply_updates(db, {"connections.alpaca_secret_key": "secretvalue1234"})

    field = _field(settings_store.public_view(db), "connections.alpaca_secret_key")
    ...
    assert settings_store.resolve(db, "connections.alpaca_secret_key") == "secretvalue1234"


def test_clearing_a_secret_drops_the_override(db):
    settings_store.apply_updates(db, {"connections.alpaca_secret_key": "abc12345"})
    settings_store.apply_updates(db, {"connections.alpaca_secret_key": ""})
    field = _field(settings_store.public_view(db), "connections.alpaca_secret_key")
...
def test_a_field_marked_unavailable_cannot_be_written(db, monkeypatch):
    ...
    field = settings_store.BY_KEY["monitor.enabled"]
